...
The current survival loop has inventory stacks, starter items, block drops, placement consumption, player stats, survival/creative/spectator movement modes, collision, gravity, jumping, underwater breath, and a first crafting menu. Offline play applies those locally; online play sends block and crafting intents to the authoritative server, which validates inventory, applies accepted world edits, and returns full inventory snapshots.

Current world content includes expanded terrain shaping, rivers, caves, ores, meadows, forests, dunes, highlands, frost peaks, mires, trees, pines, mushrooms, cacti, boulders, campsites, watchtowers, houses, desert wells, compact villages with a market stall and ambient entities. Rendering has culling, transparent water, fog, vertex AO, soft directional shading, animated water and simple animated entity rendering.
...
Inventory and HUD item sprites are rendered from `items_inventory.png` plus fallback slices from the block/nature sheets. New item icons can later be mapped in `GameSprites` without changing inventory or hotbar UI code.
